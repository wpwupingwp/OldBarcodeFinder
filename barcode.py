...
def mafft(fas_file):
    for fas in fas_file:
        run(['mafft', '--reorder --thread {0} {1} > {2}'.format(
            cpu_count(), fas, fas.replace('.fas', '.aln'))], shell=True)
...
def main():
    ...
    parser.add_argument('--cut_off', default=0.8, type=float,
                        help='''seq length < average_length*cut_off will be
                        drop off''')
    parser.add_argument('--sample', default=5, type=int,
                        help='sample selected from each group')
    parser.print_help()
